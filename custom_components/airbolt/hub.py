...
class Tracker:
    ...
    def update_device(self, discovered_info: api.FoundDevice) -> bool:
        ...
        if (
            ...
            and self._last_seen == discovered_info.last_seen_time
            and self._imei == discovered_info.imei
            and self._iccid == discovered_info.iccid
            and self._esim_iccid == discovered_info.esim.iccid
            and self._esim_eid == discovered_info.esim.eid
            and self._esim_status == discovered_info.esim.status
            and self._modem_temperature == discovered_info.modem_temperature_f
            and self._modem_state == discovered_info.modem_state
            and self._modem_voltage == discovered_info.modem_voltage
        ...
        self._last_seen = discovered_info.last_seen_time
        self._imei = discovered_info.imei
        self._iccid = discovered_info.iccid
        self._esim_iccid = discovered_info.esim.iccid
        self._esim_eid = discovered_info.esim.eid
        self._esim_status = discovered_info.esim.status
        self._modem_temperature = discovered_info.modem_temperature_f
        self._modem_state = discovered_info.modem_state
        self._modem_voltage = discovered_info.modem_voltage
        self._device_type = discovered_info.device_type
        self._operating_mode = discovered_info.operating_mode
    ...
    def update_location(self, history_entry: api.HistoryEntry) -> bool:
        ...
        self._last_seen = history_entry.time_created
        _LOGGER.debug(
        ...
        if history_entry.modem_voltage:
            self._modem_voltage = history_entry.modem_voltage
        if history_entry.modem_temperature:
            self._modem_temperature = history_entry.modem_temperature
        return True
    ...
    def modem_temperature(self) -> int | None:
        ...
        return self._modem_temperature

    @property
    def modem_voltage(self) -> int | None:
        """Voltage of the modem when the tracker last reported."""
        return self._modem_voltage
